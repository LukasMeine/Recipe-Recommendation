...
def show_recipes():
    ...
    for group in satisfied_recipes[:20]:
        ...
        for i in range(0,3):
            ...
            cur.execute("SELECT name, cuisine, provider, big_image, ingredient_amount FROM recipe_info WHERE nutrition = %s and dbscan_label = %s;", [group[i], cur_flavor, identifier])
            temp = cur.fetchall()
